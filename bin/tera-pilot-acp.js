#!/usr/bin/env node
/**
 * tera-pilot-acp — Launch the Tera Pilot ACP (Agent Client Protocol) server.
 *
 * Runs `python -m tera_pilot.agent.acp_server [args...]`. The Python
 * interpreter is resolved in this order: TERA_PILOT_PYTHON → the npm-managed
 * venv created by scripts/postinstall.js (~/.tera_pilot/venv) → system
 * python3/python.
 */
'use strict';
...
const MODULE = 'tera_pilot.agent.acp_server';
...
child.on('error', (err) => {
  console.error('Failed to start tera-pilot-acp:', err.message);
  process.exit(1);
});
