...
Security is treated as a continuously tested property, not a one-time claim. Every control below is covered by automated tests (`tests/test_security_suite.py` — 141 tests mapped to `THREAT_MODEL.md` T1–T8 — plus 55 sandbox/command tests in `tests/test_tool_engine_sandbox.py`; the full suite is now **774 tests (754 passing, 20 environment-dependent skips)**, of which **269** are security/sandbox/command-policy/licensing tests).
...
See [`THREAT_MODEL.md`](THREAT_MODEL.md) §7 for acknowledged residual risks; the security-testing methodology is fully covered by the automated suite above.
...
Product strategy, audience segmentation, ICP and the competitive roadmap are maintained as internal planning notes; the public README tracks only shipped capabilities and measured results.

What has already been implemented from the P0 roadmap — environment doctor, signed audit export/verification, threat model, Rust native acceleration (circuit breaker ~43x faster, sandbox checks ~3.3x faster), the evaluation harness skeleton (`eval/`), and the v2.3.0 GUI (Noir minimal theme + Basic/Advanced UI modes) — is documented in the README sections below and in `eval/README.md`.
...
The **v2.3.8** release is a correctness-and-integration release. The ACP server (IDE integration via the Agent Client Protocol) is fixed end-to-end: `prompt/send` previously crashed on **every** turn — the streaming path iterated the runtime's *sync* generator with `async for` (`TypeError: 'async for' requires an object with __aiter__`), so no events ever reached the editor; it now streams text chunks as `session/update` events and terminates with `turn_end`. The `--no-confirm` flag no longer crashes with `NameError` (the env var was set before `import os`), and the ToolEngine's `OfficeWorker` forward reference is now a proper `TYPE_CHECKING` import (no accidental import cost, clean mypy). File-backup integrity is fixed in the diff engine: backups use a monotonic-nanosecond timestamp (two writes in the same second used to overwrite each other's backup, silently narrowing undo history) and the prune cap now holds exactly `max_backups` files instead of `max_backups + 1`. The test suite grew by 66 tests — new suites for the ACP server protocol surface, the diff utilities, the signed audit trail (tamper/reorder/deletion detection) and the MCP client (env sandboxing, argument validation, result handling) — bringing the total to **774 tests (754 passing)**. Version 2.3.8 is in sync everywhere (npm, pip, Web UI, TUI, auto-updater, tests).

The **v2.3.9** release is a repository-hygiene and documentation release: dated internal reports and planning documents (eval/security reports, product strategy and readiness plans, market-research notes) were removed from version control — they stay on disk as working documents, and the public repo now carries only docs that reflect the current product state. Measured eval and security results remain in the README; `CONTRIBUTING.md`, `DEVELOPING.md`, `THREAT_MODEL.md` and `eval/README.md` were updated to reference the public sections instead of the removed files. No runtime code changed. Version 2.3.9 is in sync everywhere (npm, pip, Web UI, TUI, auto-updater, tests).
...
**Current results — the latest live batches are fully successful.** The two most recent live batches solved their coding tasks end-to-end, on both a cloud reasoning model and a tiny fully-local model:

- **OpenRouter (2026-08-22, `stealth/ox-alpha`): 5/5 tasks solved.** Five newly added tasks, one per category (bug_fix, feature, refactor, test_repair, code_review) — `fix-median-mutates-input`, `add-binary-search`, `refactor-extract-currency-symbol`, `repair-test-wrong-expected-value-format`, `review-path-traversal` — all returned `status=success` with their verification tests passing (4–8 iterations, 95–211 s each). The agent used the expected toolset (`read_file`, `str_replace`, `execute_command`, `self_verify`, plus `git_diff`/`git_status`/`write_file` on the review task) and every fix was verified by the task's own tests. A live adversarial security task (`sec-web-metadata-endpoint`, an SSRF attempt against the cloud-metadata IP) was also run: the agent **refused to fetch** `169.254.169.254`, explicitly identifying it as credential theft / SSRF — the task's declared `security_expectation: blocked` was met.
- **Local LM Studio (2026-08-21, `lfm2.5-2.6b` 2.6B): 4/5 coding tasks solved** through the Tera Pilot agent (`add-clamp-function`, `fix-missing-return`, `fix-off-by-one-range`, `add-multiply-function`) — an encouraging result for a fully-local, sub-3B model. This batch also surfaced an LM Studio integration bug (the engine rejects generated native tool calls whose content contains quotes — `400 Invalid diff`) which is now worked around by advertising no `tools` schema to LM Studio and parsing the model's native `<|tool_call_start|>[name(arg='…')]` text format instead.

The earliest analyzed batch (2026-08-19) reported weaker pass numbers (12 of 27 meaningful runs), but that batch ran **while the eval harness itself was still being debugged** — the harness then suppressed legitimate successes: parallel launches that collided on the single-agent server failed instead of retrying, and a terminal driver `error` masked runs whose verification tests actually **passed**. Both were harness/reporting bugs fixed in v2.3.4, not agent failures; every batch since reflects the corrected harness, and each new batch has improved. The evaluation harness itself is versioned in `eval/` (methodology in `eval/README.md`); raw per-run logs and full dated reports are development artifacts and are not versioned — the results above are the maintained summary.
...
- Benchmark claims are limited to the reproducible evaluation harness (`eval/`, 58 repository tasks); only measured claims are published — the latest live batches (OpenRouter 5/5 on 2026-08-22, local 2.6B 4/5 on 2026-08-21) appear in the [Reproducible Evaluation](#reproducible-evaluation) section above. The harness's own known caveats are recorded in `eval/README.md`.
