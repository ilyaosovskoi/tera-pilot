"""Streaming think-block scrubber (ported from Nous Research's hermes-agent,
MIT license — https://github.com/NousResearch/hermes-agent).

Strips reasoning/thinking blocks (``<think>``, ``<thinking>``,
``<reasoning>``, ...) from STREAMED model text. Per-delta regex stripping
is fragile when a tag is split across chunks, so this state machine
holds back partial tags at delta boundaries and suppresses everything
inside a reasoning block, flushing any held-back prose that turned out
not to be a real tag at end-of-stream.

Wired into OpenAICompatProvider.stream() so the TUI/agent token stream
never leaks a reasoning model's private thinking into the visible chat.
